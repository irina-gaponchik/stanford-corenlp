...
public class Timex implements Serializable {
  ...
  public Element toXmlElement() {
    ...
    if (tid != null) {
      element.addAttribute(new Attribute("tid", tid));
    }
    if (value() != null) {
      element.addAttribute(new Attribute("value", val));
    }
    if (altVal != null) {
      element.addAttribute(new Attribute("altVal", altVal));
    }
    if (type != null) {
      element.addAttribute(new Attribute("type", type));
    }
    if (beginPoint != -1) {
      element.addAttribute(new Attribute("beginPoint", "t" + String.valueOf(beginPoint)));
    }
    if (endPoint != -1) {
      element.addAttribute(new Attribute("endPoint", "t" + String.valueOf(endPoint)));
    }
    if (text != null) {
