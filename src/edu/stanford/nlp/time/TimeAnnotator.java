 ...
 * The expressions recognized by SUTime is loosely based on GUTIME.
 *
 * After annotation, the {@link TimeAnnotations.TimexAnnotations} annotation will be populated
 *   with a <code>List&lt;CoreMap&gt;</code>, each of which will represent
 *   one temporal expression.
 *
 * If a reference time is set (via {@link edu.stanford.nlp.ling.CoreAnnotations.DocDateAnnotation}), then
 *   temporal expressions are resolved with respect the to document date.
 * <p>
 * <br>
...
public class TimeAnnotator implements Annotator {
  ...
  public TimeAnnotator() {
    timexExtractor = new TimeExpressionExtractor();
  }

  public TimeAnnotator(String name, Properties props) {
    timexExtractor = new TimeExpressionExtractor(name, props);
  }
  ...
  public void annotate(Annotation annotation) {
    ...
    List<CoreMap> allTimeExpressions = null;
    List<CoreMap> sentences = annotation.get(CoreAnnotations.SentencesAnnotation.class);
    List<CoreMap> allNumerics = null;
    if (sentences != null) {
      allTimeExpressions = new ArrayList<CoreMap>();
      allNumerics = new ArrayList<CoreMap>();
