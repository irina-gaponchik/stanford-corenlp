...
public class CRFClassifier<IN extends CoreMap> extends AbstractSequenceClassifier<IN> {
  ...
  protected void printFeatures() {
    ...
    try {
      ...
      PrintWriter pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream("feats-" + flags.printFeatures
          + ".txt"), enc), true);
  ...
  public void classifyAndWriteAnswers(Collection<List<IN>> documents,
      List<Pair<int[][][], int[]>> documentDataAndLabels, PrintWriter printWriter,
  ...
  public SequenceModel getSequenceModel(List<IN> doc, Pair<int[][][], int[]> documentDataAndLabels) {
    ...
    CRFCliqueTree cliqueTree = CRFCliqueTree.getCalibratedCliqueTree(weights, data, labelIndices, classIndex.size(),
        classIndex, flags.backgroundSymbol);
  ...
  public void printProbsDocument(List<IN> document) {
    ...
    // CRFLogConditionalObjectiveFunction.getCalibratedCliqueTree(weights, data,
    // labelIndices, classIndex.size());
    CRFCliqueTree cliqueTree = CRFCliqueTree.getCalibratedCliqueTree(weights, data, labelIndices, classIndex.size(),
        classIndex, flags.backgroundSymbol);
  ...
  public CRFCliqueTree getCliqueTree(List<IN> document) {
    ...
    // CRFLogConditionalObjectiveFunction.getCalibratedCliqueTree(weights, data,
    // labelIndices, classIndex.size());
    return CRFCliqueTree.getCalibratedCliqueTree(weights, data, labelIndices, classIndex.size(), classIndex,
   ...
   *          ObjectBank to Collection for generality (mihai)
   */
  @Override
  ...
  protected Minimizer getMinimizer(int featurePruneIteration, Evaluator[] evaluators) {
    ...
    if (flags.useQN) {
      ...
      StochasticInPlaceMinimizer sgdMinimizer = new StochasticInPlaceMinimizer(flags.sigma, flags.SGDPasses,
          flags.tuneSampleSize);
      if (flags.useSGDtoQN) {
  ...
  public static CRFClassifier getClassifier(String loadPath) throws IOException, ClassCastException,
      ClassNotFoundException {
    CRFClassifier crf = new CRFClassifier();
  ...
  public static CRFClassifier getClassifier(String loadPath, Properties props) throws IOException, ClassCastException,
      ClassNotFoundException {
    CRFClassifier crf = new CRFClassifier();
