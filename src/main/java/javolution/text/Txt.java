 ...
 *     O[n] for TxtBuffer/StringBuilder).</p>
 ...
 *          {@link TxtBuffer}/{@link StringBuilder} in order to manipulate
 ...
 *     </ul></p>
 * <p> {@link javolution.text.Txt} literals should be explicitly {@link #intern interned}.
 *     Unlike strings literals and strings-value constant expressions,
 *     interning is not implicit. For example:[code]
 *         final static Txt TRUE = Txt.intern("true");
 *         final static Txt FALSE = Txt.intern("false");
 *     [/code]</p>
 *
 * <p><i> Implementation Note: To avoid expensive copy operations ,
 *        {@link javolution.text.Txt} instances are broken down into smaller immutable
 ...
 *        <code>TxtBuffer/StringBuilder</code>.</i></p>
 *
...
public   class Txt implements CharSequence, Comparable, XMLSerializable,
        ValueType {
     ...
     */
    public static final Txt EMPTY = Txt.intern("");
     ...
     */
    private Txt(boolean isPrimitive) {
    ...
    public Txt(String str) {
        ...
        if (_data != null) { // Primitive.
            ...
            _head = new Txt(str.substring(0, half));
            _tail = new Txt(str.substring(half, _count));
        }
    }
     ...
     */
    public static Txt valueOf(Object obj) {
        if (obj instanceof Number) // Use faster primitive formatting.
            return Txt.valueOfNumber(obj);
        return Txt.valueOf(String.valueOf(obj));
    }

    // For Integer, Long, Float and Double use direct formatting.
    private static Txt valueOfNumber(Object num) {
        if (num instanceof Integer)
            return Txt.valueOf(((Integer) num).intValue());
        if (num instanceof Long)
            return Txt.valueOf(((Long) num).longValue());
        if (num instanceof Float)
            return Txt.valueOf(((Float) num).floatValue());
        if (num instanceof Double)
            return Txt.valueOf(((Double) num).doubleValue());
        return Txt.valueOf(String.valueOf(num));
    }

    private static Txt valueOf(String str) {
        return Txt.valueOf(str, 0, str.length());
    }

    private static Txt valueOf(String str, int start, int end) {
        int length = end - start;
        if (length <= BLOCK_SIZE) {
            Txt text = newPrimitive(length);
            str.getChars(start, end, text._data, 0);
            return text;
        } else { // Splits on a block boundary.
            int half = ((length + BLOCK_SIZE) >> 1) & BLOCK_MASK;
            return newComposite(Txt.valueOf(str, start, start + half), Txt
     ...
     */
    public static Txt valueOf(char[] chars) {
        return Txt.valueOf(chars, 0, chars.length);
    }
     ...
     */
    public static Txt valueOf(char[] chars, int offset, int length) {
        if ((offset < 0) || (length < 0) || ((offset + length) > chars.length))
            throw new IndexOutOfBoundsException();
        if (length <= BLOCK_SIZE) {
            Txt text = Txt.newPrimitive(length);
            System.arraycopy(chars, offset, text._data, 0, length);
            return text;
        } else { // Splits on a block boundary.
            int half = ((length + BLOCK_SIZE) >> 1) & BLOCK_MASK;
            return Txt.newComposite(Txt.valueOf(chars, offset, half), Txt
     ...
     */
    static Txt valueOf(TxtBuilder tb, int start, int end) {
        int length = end - start;
        if (length <= BLOCK_SIZE) {
            Txt text = Txt.newPrimitive(length);
            ...
            int half = ((length + BLOCK_SIZE) >> 1) & BLOCK_MASK;
            return Txt.newComposite(Txt.valueOf(tb, start, start + half),
                    Txt.valueOf(tb, start + half, end));
        }
    }
     ...
     */
    public static Txt valueOf(boolean b) {
        return b ? TRUE : FALSE;
    }

    private static final Txt TRUE = Txt.intern("true");

    private static final Txt FALSE = Txt.intern("false");
     ...
     */
    public static Txt valueOf(char c) {
        Txt text = Txt.newPrimitive(1);
        text._data[0] = c;
        return text;
    }
     ...
     * @return the corresponding text instance.
     */
    public static Txt valueOf(int i) {
        TxtBuilder tb = new TxtBuilder();
     ...
     * @return the corresponding text instance.
     */
    public static Txt valueOf(int i, int radix) {
        TxtBuilder tb = new TxtBuilder();
     ...
     * @return the corresponding text instance.
     */
    public static Txt valueOf(long l) {
        TxtBuilder tb = new TxtBuilder();
     ...
     * @return the corresponding text instance.
     */
    public static Txt valueOf(long l, int radix) {
        TxtBuilder tb = new TxtBuilder();
     ...
     * @return the corresponding text instance.
     */
    public static Txt valueOf(float f) {
        TxtBuilder tb = new TxtBuilder();
    ...
    public static Txt valueOf(Txt t) {
        return t;
    }
     ...
     * @return the corresponding text instance.
     */
    public static Txt valueOf(double d) {
        TxtBuilder tb = new TxtBuilder();
     ...
     */
    public static Txt valueOf(double d, int digits, boolean scientific,
                               boolean showZero) {
        TxtBuilder tb = new TxtBuilder();
     ...
     * @return <code>this.concat(Txt.valueOf(obj))</code>
     */
    public Txt plus(Object obj) {
        return this.concat(Txt.valueOf(obj));
    }
     ...
     * @return <code>this.concat(Txt.valueOf(obj))</code>
     */
    public Txt plus(CharSequence str) {

        Txt merge = this.append(String.valueOf(str));
        return merge != null ? merge : concat(Txt.valueOf(str));
    }

    private Txt append(String str) { // Try to append, returns null if cannot.
        int length = str.length();
        if (_data == null) {
            Txt merge = _tail.append(str);
            return merge != null ? Txt.newComposite(_head, merge) : null;
        } else { // Primitive.
            if (_count + length > BLOCK_SIZE) return null; // Cannot merge.
            Txt text = Txt.newPrimitive(_count + length);
     ...
     * <code>TxtBuffer.append(String)</code>) and still returns
     * a text instance with an internal binary tree of minimal depth!
     *
     * @param  that the text that is concatenated.
     * @return <code>this + that</code>
     */
    public Txt concat(Txt that) {
        // All Txt instances are maintained balanced:
        ...
        if (length <= BLOCK_SIZE) { // Merges to primitive.
            ...
            return Txt.newComposite(head, tail);
        }
    }

    private Txt rightRotation() {
        ...
        Txt B = P._tail;
        Txt C = this._tail;
        return Txt.newComposite(A, Txt.newComposite(B, C));
    }

    private Txt leftRotation() {
        // See: http://en.wikipedia.org/wiki/Tree_rotation
        Txt Q = this._tail;
        if (Q._data != null)
        ...
        Txt C = Q._tail;
        Txt A = this._head;
        return Txt.newComposite(Txt.newComposite(A, B), C);
    }
     ...
     */
    public Txt insert(int index, CharSequence txt) {
        return subtext(0, index).concat(Txt.valueOf(txt)).concat(subtext(index));
    }
     ...
     */
    public Txt delete(int start, int end) {
        if (start > end)
            throw new IndexOutOfBoundsException();
     ...
     */
    public Txt replace(CharSequence target, CharSequence replacement) {
        int i = indexOf(target);
        return (i < 0) ? this : // No target sequence found.
                subtext(0, i).concat(Txt.valueOf(replacement)).concat(
     ...
     * @return the resulting text.
     */
    public Txt replace(CharSet charSet, CharSequence replacement) {
     ...
     */
    public static Txt intern(final CharSequence csq) {
        Txt text = (Txt) INTERN_INSTANCES.get(csq); // Thread-Safe - No entry removed.
        return (text != null) ? text : Txt.internImpl(csq.toString());
    }
     ...
     * @return an unique text instance allocated in <code>ImmortalMemory</code>.
     */
    public static Txt intern(final String str) {
        Txt text = (Txt) INTERN_INSTANCES.get(str); // Thread-Safe - No entry removed.
        return (text != null) ? text : Txt.internImpl(str);
    }

    private static synchronized Txt internImpl(final String str) {
        ...
        return (Txt) INTERN_INSTANCES.get(str);
    }
     ...
     */
    public Txt toLowerCase() {
        if (_data == null) // Composite.
            return Txt.newComposite(_head.toLowerCase(), _tail.toLowerCase());
        Txt text = Txt.newPrimitive(_count);
        for (int i = 0; i < _count;) {
     ...
     *         (start > end) || (end > this.length())</code>
     */
    public Txt subtext(int start, int end) {
        if (_data != null) { // Primitive.
    ...
    public Txt copy() {
        if (_data != null) { // Primitive.
            ...
            return Txt.newComposite((Txt) _head.copy(), (Txt) _tail.copy());
        }
    }
     ...
     */
    public static Txt valueOf(char c, int length) {
        if (length < 0)
            throw new IndexOutOfBoundsException();
        if (length <= BLOCK_SIZE) {
            Txt text = Txt.newPrimitive(length);
            ...
            return Txt.newComposite(Txt.valueOf(c, middle), Txt.valueOf(c,
                    length - middle));
        }
    }
     ...
     * @throws an IllegalArgumentException if the <code>(len<0)</code>.
     */
    public Txt padLeft(int len, char c) {
        final int padSize = (len <= length()) ? 0 : len - length();
        return insert(0, Txt.valueOf(c, padSize));
    }
     ...
     * @throws an IllegalArgumentException if the <code>(len<0)</code>.
     */
    public Txt padRight(int len, char c) {
        final int padSize = (len <= length()) ? 0 : len - length();
        return concat(Txt.valueOf(c, padSize));
    }
     ...
     */
    private static Txt newPrimitive(int length) {
        Txt text = new Txt(true);
        text._count = length;
        return text;
    }
     ...
     */
    private static Txt newComposite(Txt head, Txt tail) {
        Txt text = new Txt(false);
