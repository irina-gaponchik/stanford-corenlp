...
public class TreeBinarizer implements TreeTransformer {
  ...
  private boolean doSelectiveSplit;
  ...
  Tree binarizeLocalTree(Tree t, int headNum, TaggedWord head) {
    ...
    if (markovFactor) {
      ...
      t.setLabel(newLabel);
      Tree t2;
        t2 = insideFactor ? markovInsideBinarizeLocalTreeNew(t, headNum, 0, t.numChildren() - 1, true) : markovOutsideBinarizeLocalTree(t, head, headNum, topCat, new LinkedList<Tree>(), false);
  ...
  private Label makeSyntheticLabel(Tree t, int left, int right, int headLoc, int markovOrder) {
    ...
    if (simpleLabels) {
      result = makeSimpleSyntheticLabel(t);
        ...
        result = useWrappingLabels ? makeSyntheticLabel2(t, left, right, headLoc, markovOrder) : makeSyntheticLabel1(t, left, right, headLoc, markovOrder);
  ...
  private static Label makeSyntheticLabel1(Tree t, int left, int right, int headLoc, int markovOrder) {
    ...
    String leftString;
      leftString = left == 0 ? "[ " : " ";
    String rightString;
      rightString = right == children.length - 1 ? " ]" : " ";
