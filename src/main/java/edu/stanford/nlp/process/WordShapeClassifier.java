...
import static javolution.util.FastComparator.DIRECT;
...
public class WordShapeClassifier {
    ...
    private static String wordShapeChris2Short(CharSequence s, int len, Collection<String> knownLCWords) {
        ...
        TextBuilder sb = new TextBuilder(sbLen);
        boolean nonLetters = false;
    ...
    private static String wordShapeChris2Long(CharSequence s, boolean omitIfInBoundary, int len, Collection<String> knownLCWords) {
        ...
        FastSet<Character> seenSet;// TreeSet guarantees stable ordering; has no size parameter
        seenSet = FastSet.newInstance();
        seenSet.setValueComparator(DIRECT);
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!Character.isDigit(m))
                m = Character.isLowerCase(m) ? 'x' : Character.isUpperCase(m) || Character.isTitleCase(m) ? 'X' : 'd';
            int iIncr = 0;
            for (String gr : greek)
                if (s1.startsWith(gr, i)) {
                    m = 'g';
                    //System.out.println(s + "  ::  " + s.substring(i+1));
                    iIncr = gr.length() - 1;
                    break;
            ...
            if (i < BOUNDARY_SIZE) beginChars[beginUpto++] = m;
            else if (i < len - BOUNDARY_SIZE) seenSet.add(m);
        ...
        try {
            ...
            if (!omitIfInBoundary) {
                for (Character chr : seenSet) sb.append(chr.charValue());
            } else {
                for (FastCollection.Record tail = seenSet.tail(), e = seenSet.head().getNext(); e != e.getNext(); e = e.getNext()) {
                    Character chr = seenSet.valueOf(e);
                    char ch = chr;
                    boolean insert = true;
                    ...
                    if (insert) sb.append(ch);
                }
            }
            // and add end ones
            sb.append(endChars, 0, endUpto);

            if (knownLCWords != null && !nonLetters && knownLCWords.contains(String.valueOf(s).toLowerCase()))
                sb.append('k');
            // System.out.println(s + " became " + sb);
            return sb.toString();
        } finally {
            TextBuilder.recycle(sb);
            FastSet.recycle(seenSet);
        }
    }
    ...
    private static String wordShapeChris4Long(CharSequence s, boolean omitIfInBoundary, int len, Collection<String> knownLCWords) {
        TextBuilder sb = new TextBuilder(s.length() + 1);
        TextBuilder endSB = new TextBuilder(BOUNDARY_SIZE);
        Set<Character> boundSet = new FastSet<>(BOUNDARY_SIZE << 1);
